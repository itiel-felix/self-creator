...
const initializeCache = () => {
    if (!fs.existsSync('./cache'))
        fs.mkdirSync('./cache');
    if (!fs.existsSync('./cache/frame_info.json'))
        fs.writeFileSync('./cache/frame_info.json', '{}');
    if (!fs.existsSync('./temp'))
        fs.mkdirSync('./temp');
    if (!fs.existsSync('./temp/youtube'))
        fs.mkdirSync('./temp/youtube');
    if (fs.existsSync('./cache/videoInfo'))
        fs.rmdirSync('./cache/videoInfo', { recursive: true });
    if (fs.existsSync('./output/subtitles.ass'))
        fs.rmSync('./output/subtitles.ass', { recursive: true });
};
export { isVideoVertical, createVideoUrl, timeToSeconds, getFrameIntervalSeconds, frameIndexToSeconds, getStartAndEndTimeFromVideoId, initializeCache };
