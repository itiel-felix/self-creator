...
ffmpeg.setFfprobePath((ffprobePath as { path: string }).path);
...
function getVideoFiltersForLayout(): string[] {
    return [
...
export const cutAndConcatSegments = async (videos: VideoSegment[], outputFile: string): Promise<string> => {
        ...
        for (let i = 0; i < videos.length; i++) {
            ...
            await new Promise<void>((resolve, reject) => {
                ffmpeg(video.video_path)
                    .setStartTime(video.start_time as number)
...
export const mergeSegmentsToVerticalScreen = async (videos: VideoSegment[], outputFile: string, audioPath: string): Promise<string> => {
    await new Promise<void>((resolve, reject) => {
        ffmpeg()
...
const addAudioToVideo = async (videoPath: string, audioPath: string): Promise<string> => {
...
export const addBurnedInASSSubtitles = async (videoPath: string, subtitlesPath: string): Promise<string> => {
    try {
