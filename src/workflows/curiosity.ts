...
export const getCuriosityVideos = async (mainIdeasOriginal: any, typeOfVideo: string): Promise<VideoSegmentOut[]> => {
    const shouldUseMainIdeasCache = true;
    let mainIdeas: MainIdeaSegment[] = [];
    if (shouldUseMainIdeasCache) {
        mainIdeas = JSON.parse(fs.readFileSync('./cache/mainIdeas.json', 'utf8'));
    } else {
        mainIdeas = mainIdeasOriginal;
    }
    const videos: VideoSegmentOut[] = [];
    const tempFolder = './temp';
    if (!fs.existsSync(tempFolder)) fs.mkdirSync(tempFolder);

    const videoByKeyword: Record<string, string> = {};
    const usedVideosIds: string[] = [] as string[];

    let results: (VideoResult | string)[] = [];
    ...
    if (fs.existsSync('./cache/results.json') && shouldUseResults) {
        ...
        for (let index = 0; index < mainIdeas.length; index++) {
            ...
            const result = await chooseVideoFromYoutube(mainIdea, usedVideosIds, typeOfVideo) as VideoResult | { videoId: null; tooHard: boolean };
            results.push(result as VideoResult | string);
        }
        fs.writeFileSync('./cache/results.json', JSON.stringify(results, null, 2));
    }

    interface NewMainIdea extends MainIdeaSegment {
        video_id: string;
    ...
    const newMainIdeas: NewMainIdea[] = [];
    for (let i = 0; i < mainIdeas.length; i++) {
        ...
        const res = result as VideoResult;
        ...
        const raw = results[i];
        const vid = typeof raw === "string" ? raw : (raw as VideoResult)?.videoId;
    ...
    try {
        ...
        if (shouldDownloadVideos) {
            ...
            const downloadFinalVideo = async (_videoId: string) => { /* noop */ };
