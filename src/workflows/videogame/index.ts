...
export const getVideoGameVideos = async (videoGameName: string, _typeOfVideo?: string): Promise<any[]> => {
    ...
    for (const searchQuery of searchQueries.search_queries) {
        ...
        analyzedVideos = [...analyzedVideos, ...videos.map((video: any) => video.id ?? video.videoId)];
...
const searchVideos = async ({ searchQuery, maxVideos, round, analyzedVideos }: { searchQuery: string; maxVideos: number; round: number; analyzedVideos: string[] }) => {
