...
export const transcribeAudioWhisperX = async (audioPath: string): Promise<any> => {
...
export const transcribeAudioWhisper = async (audioPath: string): Promise<any> => {
    const cacheFile = `cache/${audioPath.split('/').pop()!.split('.')[0]}.json`;
    if (fs.existsSync(cacheFile)) {
        return JSON.parse(fs.readFileSync(cacheFile, 'utf8'));
    }
