...
export const getMainIdea = async (transcriptionOfAudioInSrtFormat: string, typeOfVideo: string = "curiosity"): Promise<any> => {
    ...
    try {
        ...
        throw new Error(`Failed to get main idea: ${(error as Error).message}`);
    }
}


export const getProcessedMainIdea = async (mainIdeaText: string, tooHard: boolean = false, burnedTerms: string[] = [], mainIdeaOriginalText: string, typeOfVideo: string = "curiosity"): Promise<any> => {
    const openai = new OpenAI({
        baseURL: 'https://api.deepseek.com',
        apiKey: process.env.DEEPSEEK_API_KEY,
    });
    ...
    try {
        ...
        throw new Error(`Failed to get processed main idea: ${mainIdeaText}: ${(error as Error).message}`);
    }
}

export const getSearchQueries = async (videoGameName: string, banned_terms: string[] = []): Promise<any> => {
    const openai = new OpenAI({
        baseURL: 'https://api.deepseek.com',
        apiKey: process.env.DEEPSEEK_API_KEY,
    });
    try {
        ...
        throw new Error(`Failed to get search queries: ${videoGameName}: ${(error as Error).message}`);
    }
}
